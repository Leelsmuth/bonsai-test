  ...
  return (
    <div className="product-card-container product-card-container--parent">
      ...
      <div className="product-card-details">
        ...
        {isDiscontinued || variants.every((variant:any) => variant.isDiscontinued) || variants.every((variant:any) => variant.quantity < 1) ? (<><h4 style={{ color: 'red' }}>Out of Stock</h4></>) : ''}
        {showInfo && (
          <div className='variant-info'>
            <div className='variant-info--price'>Price: <span>{formatter.format(variantPrice/100)}</span></div>
            <div className='variant-info--qty'><span>{variantQuantity}</span> available</div>
            <div className='variant-info--type'><span>{variantType}</span></div>
          </div>
        )}
        <div style={{ display: 'flex' }}>
          {variants.map((variantItem: any) => (
            <VariantCard key={variantItem.id} variant={variantItem} productDiscontinued={isDiscontinued} name={name} variantInfo={getVariantInfo} />
          ))}
        </div>
      </div>
      {showAddCart && <button className='add-to-cart' onClick={() => dispatch(addToCart(cartItems))}>Add to cart</button>}
